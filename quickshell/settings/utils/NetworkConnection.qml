 ...
 * NetworkConnection
 *
 * Centralized utility for wireless connection logic: a single source of truth
 * for connect/disconnect handling, saved-profile checks and password prompts.
 *
 * Usage:
 * ```qml
 * import qs.settings.utils
 *
 * // With a Session object (settings panel)
 * NetworkConnection.handleConnect(network, session);
 *
 * // Without a Session object - provide a password dialog callback
