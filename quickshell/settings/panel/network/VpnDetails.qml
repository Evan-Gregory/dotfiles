...
DeviceDetails {
    ...
    readonly property bool providerEnabled: {
        if (!vpnProvider || vpnProvider.index === undefined)
            return false;
        const provider = Config.vpn.provider[vpnProvider.index];
                ...
                SectionContainer {
                    ToggleRow {
                        ...
                        toggle.onToggled: {
                            ...
                            for (let i = 0; i < Config.vpn.provider.length; i++) {
                                const p = Config.vpn.provider[i];
                                if (typeof p === "object") {
                                    const newProvider = {
                                        name: p.name,
                            ...
                            onClicked: {
                                ...
                                for (let i = 0; i < Config.vpn.provider.length; i++) {
                                    if (i !== root.vpnProvider.index) {
                                        providers.push(Config.vpn.provider[i]);
                                    }
                                }
                                Config.vpn.provider = providers;
                                Config.save();
        ...
        contentItem: ColumnLayout {
            ...
            RowLayout {
                ...
                TextButton {
                    ...
                    onClicked: {
                        const providers = [];
                        const oldProvider = Config.vpn.provider[editVpnDialog.editIndex];
