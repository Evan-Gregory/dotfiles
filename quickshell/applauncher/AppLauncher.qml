...
Item {
    ...
    // Emitted when the launcher should dismiss (app launched or Escape). The
    // host window connects this to hide the overlay.
    ...
    // -------------------------------------------------------------------------
    // COLORS (semantic palette from Theme / colors scheme)
    // -------------------------------------------------------------------------
    Theme {
        id: _theme
    }
    ...
    Process {
        id: appFetcher
        running: true
        command: ["bash", "-c", "python3 '" + Qt.resolvedUrl("app_fetcher.py").toString().replace(/^file:\/\//, "") + "'"]
    ...
    function launchApp(execStr) {
        // This system's Hyprland uses a Lua config provider, which repurposes
        // `hyprctl dispatch` to evaluate a Lua expression (e.g.
        // hl.dsp.exec_cmd("...")) rather than the stock "<dispatcher> <args>"
        // syntax. Escape backslashes/quotes so execStr round-trips as a Lua
        // string literal.
        let escaped = execStr.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
        Quickshell.execDetached(["hyprctl", "dispatch", 'hl.dsp.exec_cmd("' + escaped + '")']);
